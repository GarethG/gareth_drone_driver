...
//				 Basically - try and use the profile position function to drive to a postion. No homing or any smart stuff has been implemented so no random magic numbers please
...
long desiredPosition = 0; //global variable to pass data from the subscriber callback to the VCS_position function. Should be in an object rather than global, but I don't have time to learn OOcpp properly
...
{
  //I heard something - now do something about it. squirt msg into something (pretty sure Int32 is the same as a long)
	//msg->desiredPosition; //put the subscribed data into a global variable, then it's accessible in main
...
int main(int argc, char** argv)
{
	...
	//PrintSettings();
	ros::init(argc, argv, "garethEposNode"); //must do ROS init before anything else ROSy
	ros::NodeHandle n; //create a nodehandle n - node handles are the core of ros communication 
...
//start ROS Loops here 

//subscribe to a float position 


//VCS_SetPositionProfile(HANDLE KeyHandle, WORD NodeId, DWORD ProfileVelocity, DWORD ProfileAcceleration, DWORD ProfileDeceleration, DWORD* pErrorCode)
	...
	ros::spin(); //this services the callbacks hmm, maybe this should be in the publisher while loop?
